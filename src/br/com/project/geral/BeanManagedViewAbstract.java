...
import br.com.project.enumn.CondicaoPesquisa;
			...
			for (Field field : getClassImplement().getDeclaredFields()) {
				...
				if (field.isAnnotationPresent(IdentificaCampoPesquisa.class)) {
...

					{
						...
						objetoCampoConsultaSelecionado.setPrincipal(field.getAnnotation(IdentificaCampoPesquisa.class).principarl());
						break;
					}
	...
	public List<SelectItem> getListaCampoPesquisa() {
		...
		for (Field field : getClassImplement().getDeclaredFields()) {
			...
			if (field.isAnnotationPresent(IdentificaCampoPesquisa.class)) {
				...
				int isPrincipal = field.getAnnotation(IdentificaCampoPesquisa.class).principarl();
				...
				objetoCampoConsulta.setTipoClass(field.getType().getName());
